...
class Detail extends Component {
  ...
  render() {
    return (
      <div id="detail">
        <DetailNavBar titleClick={this.titleClick} clickBack={this.clickBack}></DetailNavBar>
        <Scroll
          ref = {this.scroll} 
          probType={2}
          pullUpLoad={false}>
          <DetailSwiper topImages={this.state.topImages} imgLoad={this.state.imgLoad}></DetailSwiper>
  ...
  componentDidMount(){
    if(!this.props.location.state) {this.props.history.push('/home');return;}
    ...
    getDetail(this.props.location.state.iid).then(res => {
      ...
      // 商品参数信息
      this.setState({ paramInfo: new GoodsParam(
  ...
  scrollRefresh = () => {
    this.scroll.current && this.scroll.current.state.scroll && this.scroll.current.state.scroll.refresh();
  }
